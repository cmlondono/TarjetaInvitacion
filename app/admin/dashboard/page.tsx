...
export default function PaginaAdminDashboard() {
  ...
  const cargarDatos = () => {
    ...
    setEventos(EventoRepositorio.obtenerTodos())
  }
  ...
  return (
    ...
    >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-8 py-8 space-y-8">
        ...
        {/* ═════════ PESTAÑA 4: GESTOR GLOBAL DE EVENTOS ═════════ */}
        {pestana === 'eventos' && (
          <div className="space-y-6">
            <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-2xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  <h3 className="text-lg font-bold text-slate-950 flex items-center gap-2">
                    <Calendar size={18} />
                    <span>Control Central de Eventos ({eventos.length})</span>
                  </h3>
                  <p className="text-xs text-slate-600 mt-0.5">
                    Busca eventos por título, supervisa enlaces públicos y activa licencias manualmente.
                  </p>
                </div>
                ...
                <div className="relative w-full sm:w-72">
                  <input
                    ...
                    className="w-full px-3.5 py-2 pl-9 rounded-xl border border-slate-300 bg-white text-xs text-slate-900 focus:border-slate-900 focus:outline-hidden"
                  />
                ...
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="border-b border-slate-200 text-slate-500 font-bold uppercase tracking-wider text-[10px]">
                      ...
                      <th className="py-2.5 px-3">Anfitrión</th>
                      <th className="py-2.5 px-3">Estado Plan</th>
                      <th className="py-2.5 px-3">Fecha del Evento</th>
                    ...
                    {eventosFiltrados.length === 0 ? (
                      ...
                      eventosFiltrados.map((ev) => (
                        <tr key={ev.id} className="hover:bg-slate-50/70">
                          <td className="py-3 px-3">
                            <span className="font-bold text-slate-950 block">{ev.titulo}</span>
                            <span className="font-mono text-[10px] text-slate-500">
                              /i/{ev.slugPublico}
                            </span>
                          </td>
                          <td className="py-3 px-3 text-slate-700">{ev.anfitriones || '—'}</td>
                          <td className="py-3 px-3">
                            <button
                              type="button"
                          ...
                          <td className="py-3 px-3 text-right space-x-1.5">
                            <Link
                              href={`/i/${ev.slugPublico}`}
                              target="_blank"
                              className="px-2.5 py-1 rounded-lg border border-slate-200 hover:bg-slate-100 text-slate-700 text-[11px] font-semibold inline-flex items-center gap-1"
                              title="Ver tarjeta del invitado"
                            >
                              <span>Tarjeta</span>
                            ...
                            <button
                              ...
                              className="p-1 rounded-lg text-slate-400 hover:text-rose-600 cursor-pointer transition-colors"
